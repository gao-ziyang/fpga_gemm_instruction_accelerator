# Phase 2 / Iteration 012：宫老师反馈后的理解，从功能验证转向资源-带宽-并行度权衡

这篇作为 Phase 2 的入口。前 10 次迭代主要是在证明 GEMM、CNN、Attention 和指令调度路径能跑通；从这里开始，我把重点转到老师强调的三件事：计算并行度、片上缓存容量、片外访存带宽。
