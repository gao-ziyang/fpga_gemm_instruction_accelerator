# Phase 1 / 迭代日志 003：片上 buffer、边界处理和右移量化

> 说明：这一篇记录的是当时的学习版本，所以里面还有“GEMM 写回时固定右移”的设计。后来在迭代 007 里我重新整理了这个问题，当前版本已经把固定右移从 GEMM core 里移出，GEMM 只输出原始 INT32 累加结果；需要量化时再单独做 shift + saturate。
