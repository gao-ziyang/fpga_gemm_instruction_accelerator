...
#   vitis_hls -f path/to/run_shared_gemm_tile8_split_axi_qkv_conv_fused_store_prepacked_w_csynth.tcl
#
# Stage-3c probe: Stage-3a plus prepacked Conv2D weights. The PL still
# performs im2col, GEMM, and final channel-major store, but the weights are
# already stored in GEMM KxM layout, avoiding runtime weight flatten.
...
set project_name "shared_gemm_tile8_split_axi_qkv_conv_fused_store_prepacked_w_csynth"
set cflags [format "-I%s -I%s -DGZY_GEMM_TILE=8 -DGZY_GEMM_BLOCK_M=8 -DGZY_ACCEL_BLOCK_N=16 -DGZY_ACCEL_BLOCK_K=32 -DGZY_ACCEL_BLOCK_M=32 -DGZY_ACCEL_MAX_N=64 -DGZY_ACCEL_MAX_K=128 -DGZY_ACCEL_MAX_M=1024 -DGZY_ACCEL_MAX_INSTR=32 -DGZY_ACCEL_BENCH_N=16 -DGZY_ACCEL_BENCH_K=96 -DGZY_ACCEL_BENCH_M=96 -DGZY_ACCEL_SHARED_GEMM_INLINE=1 -DGZY_ACCEL_SPLIT_AXI_BUNDLES=1 -DGZY_ACCEL_QKV_FUSED_STORE=1 -DGZY_ACCEL_CONV_FUSED_STORE=1 -DGZY_ACCEL_CONV_PREPACKED_WEIGHT=1 -DGZY_ACCEL_TB_CONV_PREPACKED_WEIGHT=1" $src_dir $tb_dir]
