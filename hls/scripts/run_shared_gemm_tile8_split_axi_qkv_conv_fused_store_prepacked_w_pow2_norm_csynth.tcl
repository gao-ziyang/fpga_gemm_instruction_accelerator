...
#   vitis_hls -f path/to/run_shared_gemm_tile8_split_axi_qkv_conv_fused_store_prepacked_w_pow2_norm_csynth.tcl
#
# Stage-4a probe: Stage-3c plus power-of-two approximate attention
# normalization. This removes the exact row-normalization divider from the
# PL-side attention path while keeping the same shared GEMM/Conv2D structure.
...
set project_name "shared_gemm_tile8_split_axi_qkv_conv_fused_store_prepacked_w_pow2_norm_csynth"
set cflags [format "-I%s -I%s -DGZY_GEMM_TILE=8 -DGZY_GEMM_BLOCK_M=8 -DGZY_ACCEL_BLOCK_N=16 -DGZY_ACCEL_BLOCK_K=32 -DGZY_ACCEL_BLOCK_M=32 -DGZY_ACCEL_MAX_N=64 -DGZY_ACCEL_MAX_K=128 -DGZY_ACCEL_MAX_M=1024 -DGZY_ACCEL_MAX_INSTR=32 -DGZY_ACCEL_BENCH_N=16 -DGZY_ACCEL_BENCH_K=96 -DGZY_ACCEL_BENCH_M=96 -DGZY_ACCEL_SHARED_GEMM_INLINE=1 -DGZY_ACCEL_SPLIT_AXI_BUNDLES=1 -DGZY_ACCEL_QKV_FUSED_STORE=1 -DGZY_ACCEL_CONV_FUSED_STORE=1 -DGZY_ACCEL_CONV_PREPACKED_WEIGHT=1 -DGZY_ACCEL_TB_CONV_PREPACKED_WEIGHT=1 -DGZY_ACCEL_ATTN_NORM_POW2_APPROX=1 -DGZY_ACCEL_TB_ATTN_NORM_POW2_APPROX=1" $src_dir $tb_dir]
