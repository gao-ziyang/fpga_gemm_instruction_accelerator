...
当前 PS-PL-DDR 的 GEMM 闭环已经比较稳，下一步应该回到 PL/HLS 端，把板级 IP 从“只验证 112”推进到“能承接更大矩阵和更多算子”。
...
1. 重新导出大尺寸兼容 AXI IP，例如先把 `GZY_ACCEL_MAX_N/K/M` 扩到 `1008` 或一个明确的大矩阵目标。
2. 先测 `1008x1008x1008`，确认大矩阵 full-block 主路径。
3. 再测 `1024x1024x1024`，专门验证边界路径。
4. 把已有 Conv / Attention 逻辑迁移到当前 `gemm_scheduler` 和 instruction stream 体系里。
5. 扩展 opcode 和 decode，让顶层能 dispatch `GEMM`、`CONV2D`、`ATTN` 或更细的 Transformer 子算子。
6. PS 端再分别验证 GEMM、Conv、Attention 三类功能。
7. 功能闭环稳定后，再集中优化 GEMM 的资源和 latency。

现在不建议一上来就为了 `2048` 改一堆东西。更稳的是先做“大尺寸 GEMM IP 能跑”，再迁移 Conv/Attention，最后做性能优化。功能边界先清楚，优化才不会把调试问题搅在一起。
